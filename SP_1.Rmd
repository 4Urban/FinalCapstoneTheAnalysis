...
getPlot <- function(data, group_var, header_str, bool_label=FALSE) {
  ...
  smmryPlot <- ggplot(smmry, aes(ymax=ymax, ymin=ymin, xmax=4, xmin=3, fill=color)) +
    geom_rect() +
    geom_text(x=7, aes(y=labelPosition, label=label, color=color), size=4, check_overlap=TRUE) +
  ...
  if(bool_label){
    if(group_var_str %in% colnames(option)){ # add information about Label(korean) of Number
      ...
      smmryTblPlot <- tableGrob(smmry[, 1:3], rows=NULL, 
                                ...
                                theme=ttheme_default(base_family="NanumGothic"))
...
getPlot(AttTrip, TripL_mode_car, "최근 3개년 내 장거리 승용차/승합차 탑승 여부", TRUE)
getPlot(AttTrip, TripL_mode_expbus, "최근 3개년 내 장거리 고속·시외버스 탑승 여부", TRUE)
getPlot(AttTrip, TripL_mode_train, "최근 3개년 내 장거리 고속철도 이외 일반철도 탑승 여부", TRUE)
getPlot(AttTrip, TripL_mode_expt, "최근 3개년 내 장거리 고속철도 탑승 여부", TRUE)
getPlot(AttTrip, TripL_mode_plane, "최근 3개년 내 장거리 국내선 항공 탑승 여부", TRUE)
getPlot(AttTrip, TripL_mode_ship, "최근 3개년 내 장거리 선박 탑승 여부", TRUE)
```
...
remove(data, dt, level, option, vars, varsP, varsE)
```
