...
getPlot <- function(data, group_var, header_str, bool_label=FALSE, label_TF) {
...
getPlot(AttPerson, Age20, "20대 여부", TRUE, "TF")
getPlot(AttPerson, HHsize, "총가구원 수(명)")
getPlot(AttPerson, Job, "직업", TRUE)
getPlot(AttPerson, Job8, "무직/학생 여부", TRUE, "TF")
getPlot(AttPerson, Inc, "월 평균 소득", TRUE)
getPlot(AttPerson, Ncars, "자동차 소유 여부", TRUE, "YN")
```
...
getPlot(AttSp %>% filter(Dist==150), Choice, , TRUE)
getPlot(AttSp %>% filter(Dist==250), Choice, , TRUE)
getPlot(AttSp %>% filter(Dist==300), Choice, , TRUE)
```
...
f_Age20_Job8 <- mFormula(Choice ~ Cost | Gender + HHsize + Job8 + Inc + Ncars + Age20 + TripL_mode_car + TripL_mode_expbus + TripL_mode_train + TripL_mode_expt + TripL_mode_plane + TripL_expt + TripL_plane + Trip_made + Conv + Eco + Safe + Trans + New + Sens | Time)
...
#### '20대 여부' 및 '무직, 학생 여부' 변수 대입

```{r}
summary(mlogit(f_Age20_Job8, dtWide, reflevel="hyper"))
summary(mlogit(f_Age20_Job8, dtWide150, reflevel="hyper"))
summary(mlogit(f_Age20_Job8, dtWide250, reflevel="hyper"))
summary(mlogit(f_Age20_Job8, dtWide300, reflevel="hyper"))
```
...
```{r}
f2 <- mFormula(Choice ~ Cost | Gender + HHsize + Job8 + Inc + Ncars + AgeI + Trip_made + Conv + Eco + Safe + Trans + New + Sens | Time)

summary(mlogit(f2, dtWide, reflevel="hyper"))
summary(mlogit(f2, dtWide150, reflevel="hyper"))
summary(mlogit(f2, dtWide250, reflevel="hyper"))
summary(mlogit(f2, dtWide300, reflevel="hyper"))
```
